...
(function ($) {
  //スムーススクロール
  $.fn.uq_smoothScroll = function (options) {
    //引数を設定する
    var defaults = {
      speed: 750,
      offset: 0,
...
(function (global) {
    ...
    var _fade = function () {
      $('.lujs-scrollfade').each(function () {
        ...
        if (_scT > _pos - _winH + _winH / 6) {
          if (_this.is(':visible')) {
            _this.addClass('lujs-scrollfade--load');
          }
        }
      });
    }
  ...
  var initRemodalYoutube = function (selector) {
    var ytplayer, //YTplayer用
      ytplayers = [], //YTplayer格納用
      playerIdArray = [], // player判別用idの配列
      movieIdArray = [], // youtube動画のid用の配列
      _index, //何番目のモーダル
      player,
      luPlayer,
      readyFlag = 0;
    ...
    if ($(selector).length) {
      ...
      var pt = initRemodalYoutube.prototype;
    ...
    return initRemodalYoutube;
  }

  $("#pai2, #pai").each(function () {
    var pai1Str = 'M0,1263.29C0,';
    var pai2Str = ',190.87,914,361.75,798.71,561,664.3,735,479.13,806.08,244.74' +
      'A 844.45,844.45,0,0,0,842.19,0' +
      'V 2160' +
      'H 285.94' +
      'l 0,0' +
      'a 390.07,390.07,0,0,0,70.26-224' +
      'C 356.22,1643.18,0,';
    var pai3Str = ',0,';
    var pai4Str = 'Z';
    var t = 0;
    ...
    function pulun() {
      var tw1 = new TWEEN.Tween(pos)
        .to({
          p0: 1032.08 * (1 - t) + 1100.08 * t,
          p1: 1562.87 * (1 - t) + 1630.87 * t,
          p2: 1263.29 * (1 - t) + 1331.29 * t,
        }, tm)
        .easing(TWEEN.Easing.Cubic.In)
        .onUpdate(function () {
          controlPath.setAttribute('d', pai1Str + this.p0 + pai2Str + this.p1 + pai3Str + this.p2 + pai4Str)
        })
        .onComplete(function () {
          var tw2 = new TWEEN.Tween(pos)
            .to({
              p0: 1032.08,
              p1: 1562.87,
              p2: 1263.29
  ...
  $(function () {
    initScrollfade();
    LUNA_MODULES.initRemodalYoutube('.lujs-remodalYoutube');
  });
  ...
  //EXPORT
  LUNA_MODULES.initRemodalYoutube = initRemodalYoutube;
...
if ($('#lu-player').length) {
  ...
  function onYouTubeIframeAPIReady() {
    luPlayer = new YT.Player('lu-player', {
      playerVars: {
        //playsinline: 1
      },
      events: {
  ...
  $('.lu-movie__main__thumb').on("click", function (e) {
    $('.lu-movie__main__thumb').fadeOut(200);
    luPlayer.playVideo();
  });

  $(".lujs-selectYoutube").on("click", function (e) {
    e.preventDefault();
    $('.lu-movie__main__thumb').fadeOut(200);
    var videoId = $(this).attr("href");
    luPlayer.loadVideoById(videoId);
  });
}